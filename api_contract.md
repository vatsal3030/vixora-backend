...
- Express is not using strict routing, so `/path` and `/path/` both resolve in practice.
...
- Frontend origin must be allowlisted in backend CORS config (`CORS_ORIGIN` / `ADMIN_FRONTEND_URL`).

Rate-limit behavior:
- Global API limiter on `/api/*`: `100 requests/minute/IP`
- Auth routes limiter: `30 requests/15 minutes/IP`
- OTP routes limiter: `10 requests/15 minutes/IP`
- AI routes limiter: `30 requests/15 minutes/IP`
- Exceeded limit response uses `success=false` and a message (`429`).
...
- category inputs (any of these):
  - `categoryId`
  - `categoryIds` (array or csv)
  - `category`
  - `categories` (array, csv, or objects with `id|slug|name`)
  - `categorySlug`
  - `categorySlugs` (array or csv)
- transcript inputs:
...
- normalized tags and resolved categories are included when provided
...
- `GET /feed/tags` (optional auth)
- `GET /feed/tags/:tagName` (optional auth)
- `GET /feed/home` (auth)
...
- `tag` supported on `GET /feed/home` for tag-filtered home feed

Tag endpoints:
- `GET /feed/tags`: ranked tag chips for home page
  - query: `page`, `limit`, `q`
  - each tag item includes `videoCount`, `scores.final`, `scores.trending`, `scores.interest`
  - low-content fallback: backend can return default topics even when no ranked tag pool exists
- `GET /feed/tags/:tagName`: feed filtered to one tag
  - query: `page`, `limit`, `sortBy=score|createdAt`, `sortType=asc|desc`
  - response `data.tag` includes tag metadata (`id`, `name`, `displayName`, `slug`)
...
- feed video items include `tags: string[]` and `categories: [{id,name,slug,icon}]`
...
- `POST /admin/feed/topics/seed` (ADMIN+)
  - purpose: bootstrap default home chip topics as both `Tag` and `Category`
  - body optional: `{ topics: string[] | "csv" }`
  - if body is empty, backend falls back to `DEFAULT_FEED_TOPICS` env or built-in defaults (`music,gaming,tech,live,news,movies`)
...
- `DEFAULT_FEED_TOPICS=music,gaming,tech,live,news,movies` (optional; used by tag-chip fallback + admin topic seed endpoint)
